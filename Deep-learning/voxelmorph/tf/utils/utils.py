...
def transform(vol, loc_shift, interp_method='linear', indexing='ij', fill_value=None):
    ...
    assert loc_shift.shape[-1] == nb_dims, \
        'Dimension check failed for ne.utils_for_mask.transform(): {}D volume (shape {}) called ' \
...
def batch_transform(vol, loc_shift,
    ...
    Other Notes:
    - we couldn't use ne.utils_for_mask.flatten_axes() because that computes the axes size from tf.shape(),
