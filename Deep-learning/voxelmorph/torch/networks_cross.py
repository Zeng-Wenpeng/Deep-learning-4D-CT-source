...
from .transformer_models.swintransformer_3d_first_cross import SwinTransformer3D
...
class VxmDense(LoadableModel):
    ...
    def forward(self, source, target, atlas_mask, registration=False):
        ...
        '''
        '''
        下面这对于cross这种方式，首先不需要进行cat
        首先分别进入一层encoder进行提取特征
        然后使用cross transformer attention
        然后进行cat再进行transformer一层
        '''
        x = self.encoder_transformer(source, target)  # [2, 16, 128, 128, 128]
