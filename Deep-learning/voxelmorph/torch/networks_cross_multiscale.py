...
from .transformer_models.swintransformer_3d_cross_multiscale import SwinTransformer3D
...
class split_conv_encoder(nn.Module):
    def __init__(self, in_channels, out_channels, base_n_filter=8):
        super(split_conv_encoder, self).__init__()
        self.in_channels = in_channels
        self.n_classes = out_channels
        self.base_n_filter = base_n_filter
...
class VxmDense(LoadableModel):
    ...
    def __init__(self,
        ...
        self.split_encoder = split_conv_encoder(
            in_channels=1,
            out_channels=16,
        )
    ...
    def forward(self, source, target, atlas_mask, registration=False):
        '''
        Parameters:
        ...
        '''
        '''
        1. 分别进入modified的encoder部分
        2. encoder在各个阶段的输出进入cross attention
        '''
        mov_outputs = self.split_encoder(source)
        fix_outputs = self.split_encoder(target)


        x = self.encoder_transformer(mov_outputs, fix_outputs)  # [2, 16, 128, 128, 128]
...
if __name__ == '__main__':
    ...
    print(test_model)
    mov = torch.rand(1, 1, 128, 128, 128).cuda()
    fix = torch.rand(1, 1, 128, 128, 128).cuda()
    output = test_model(mov, fix, atlas_mask=None, registration=False)
    print(output.size())
